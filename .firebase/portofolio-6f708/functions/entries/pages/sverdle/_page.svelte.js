import { c as create_anchor, d as ensure_array_like, h as head, a as attr, f as stringify, e as escape, i as bind_props, p as pop, b as push } from "../../../chunks/index.js";
import "devalue";
...
function _page($$payload, $$props) {
  ...
  $$payload.out += `<h1 class="visually-hidden svelte-1pg2j5l">Sverdle</h1> <form method="POST" action="?/enter" class="svelte-1pg2j5l"><a class="how-to-play svelte-1pg2j5l" href="/sverdle/how-to-play">How to play</a> <div${attr(
    "class",
  ...
  for (let $$index_1 = 0; $$index_1 < each_array.length; $$index_1++) {
    ...
    $$payload.out += `${anchor_1}<h2 class="visually-hidden svelte-1pg2j5l">Row ${escape(row + 1)}</h2> <div${attr("class", `row svelte-1pg2j5l ${stringify([current ? "current" : ""].filter(Boolean).join(" "))}`, false)}>${anchor_2}`;
    for (let $$index = 0; $$index < each_array_1.length; $$index++) {
      ...
      $$payload.out += `${anchor_3}<div${attr(
        ...
        false
      )}>${escape(value)} <span class="visually-hidden svelte-1pg2j5l">${anchor_4}`;
      if (exact) {
      ...
      $$payload.out += `${anchor_4}</span> <input name="guess"${attr("disabled", !current, true)} type="hidden"${attr("value", value, false)} class="svelte-1pg2j5l"></div>${anchor_3}`;
    }
  ...
  if (won || data.answers.length >= 6) {
    ...
    if (!won && data.answer) {
      $$payload.out += "<!--ssr:if:true-->";
      $$payload.out += `<p class="svelte-1pg2j5l">the answer was "${escape(data.answer)}"</p>`;
    } else {
      $$payload.out += "<!--ssr:if:false-->";
    }
